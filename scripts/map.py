...
class Map:
    ...
    def __collision_handler(self, new_map: Map) -> None:
        ...
        while collision_positions:
            ...
            collisions_here = self.__collision_sorter(new_map, current_collision_position)
            while collisions_here:
                current_collision = collisions_here.pop()
                entity_strategies = [Entity.get_strategy_name() for Entity in current_collision]
                try:
                    temp_fn = getattr(collision_behaviours, "_".join(entity_strategies))
                except AttributeError:
                    try:
                        #Perhaps check for generic behaviours (ie walls) here?
                        temp_fn = getattr(collision_behaviours, "_".join(reversed(entity_strategies)))
                    except AttributeError:
    ...
    def __collision_detector(self, new_map: Map) -> list[Point]:
        ...
        collision_locs = [Point(x, y) for x in range(new_map.get_nrows()) for y in range(new_map.get_ncols())]
        return collision_locs


    def __collision_sorter(self, new_map: Map, current_collision_position: Point) -> list[tuple]:
        """
        ...
        remaining_pairs = [(creatures[x], creatures[y]) for x in range(num_creatures) for y in range(x+1, num_creatures)]
